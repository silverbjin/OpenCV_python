...
# 3단계: 픽셀 쌍을 선택하고, BRIEF 디스크립터를 수동으로 생성 (256비트)
def generate_brief_descriptor(patch, num_bits=256):
    ...
    # 256개의 비트를 32개의 8비트 정수로 변환
    descriptor_8bit = []
    for i in range(0, len(descriptor), 8):
        # 이진 비트 8개를 모아 8비트 정수로 변환
        byte = descriptor[i:i+8]
        byte_str = ''.join(map(str, byte))
        descriptor_8bit.append(int(byte_str, 2))

    return np.array(descriptor_8bit, dtype=np.uint8)

# 4단계: BRIEF 디스크립터 생성 (256비트)
brief_descriptor = generate_brief_descriptor(patch)

# 5단계: 디스크립터 출력 (32개의 8비트 값)
print("Generated 256-bit BRIEF Descriptor as 32 bytes (8-bit each):")
